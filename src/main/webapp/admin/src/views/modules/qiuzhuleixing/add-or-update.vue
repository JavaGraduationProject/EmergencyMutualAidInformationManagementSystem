<template>
  <div class="addEdit-block">
    ...
    >
      <el-row >
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="求助类型" prop="qiuzhuleixing">
          <el-input v-model="ruleForm.qiuzhuleixing" 
              placeholder="求助类型" clearable  :readonly="ro.qiuzhuleixing"></el-input>
        </el-form-item>
        <div v-else>
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `qiuzhuleixing/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.qiuzhuleixingCrossAddOrUpdateFlag = false;
						 this.parent.search();
