<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'" label="志愿账号" prop="zhiyuanzhanghao">
          <el-select :disabled="ro.zhiyuanzhanghao" @change="zhiyuanzhanghaoChange" v-model="ruleForm.zhiyuanzhanghao" placeholder="请选择志愿账号">
            <el-option
                v-for="(item,index) in zhiyuanzhanghaoOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input"v-if="ruleForm.zhiyuanzhanghao" label="志愿账号" prop="zhiyuanzhanghao">
...
export default {
  ...
  methods: {
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/zhiyuanzhe/zhiyuanzhanghao`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.zhiyuanzhanghaoOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
    ...
    // 下二随
    zhiyuanzhanghaoChange () {
      this.$http({
        url: `follow/zhiyuanzhe/zhiyuanzhanghao?columnValue=`+ this.ruleForm.zhiyuanzhanghao,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
              if(data.data.zhiyuanxingming){
                this.ruleForm.zhiyuanxingming = data.data.zhiyuanxingming
              }
        } else {
          this.$message.error(data.msg);
        }
	...
	this.ruleForm.neirong = this.ruleForm.neirong.replace(reg,'../../../ssmb06jd/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
